...
#define RESCAN(...)		__VA_ARGS__

/* Define an XMacro for file-related functions */
#define XMACRO_FILE(F) \
	F(FILE_A)	F(FILE_B)	F(FILE_C)	F(FILE_D)		\
	F(FILE_E)	F(FILE_F)	F(FILE_G)	F(FILE_H)

/* Define an XMacro for rank-related functions */
#define XMACRO_RANK(F) \
	F(RANK_1)	F(RANK_2)	F(RANK_3)	F(RANK_4)		\
	F(RANK_5)	F(RANK_6)	F(RANK_7)	F(RANK_8)
